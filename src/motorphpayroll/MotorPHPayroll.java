 ...
 * Automates the calculation of employee salaries. This reads raw data from CSV files, applies deductions, and calculates withholding tax to get the accurate net pay. 
 * 
...
import java.io.BufferedReader; //To read files line by line efficiently
import java.io.FileReader; //To open the connection to the .csv file
import java.io.IOException; //To handle potential input/output failures, such as a missing file or a restricted directory
import java.util.Scanner; //To capture user input from the console
import java.time.LocalTime; //To represent time (HH:mm) used for time-based logic (grace periods)
import java.time.format.DateTimeFormatter; //To standardize how time is displayed or parsed, ensuring user input matches the required 24 hour pattern
import java.util.ArrayList; //To use a list that will be automatically grow size if we add more records in csv files
