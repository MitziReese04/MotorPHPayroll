...
6. **handlePayrollStaffFlow** - manages the primary navigation menu for Payroll Staff, providing options to enter the payroll processing section or exit the system.
...
9. **processPayrollMenu** - A sub-menu for staff that allows for the selection of either "One employee" or "All employees" when generating payroll reports.
Nested control flow

10. **monthName** - utility that helps convert numeric month inputs (e.g., "06") into their corresponding names (e.g., "June").
Uses switch expression

11. **processAll** -  reads the entire employee.csv file and automatically runs the calculatePayroll function for every active record for the chosen month.

12. **calculateShift** - Determines the total hours worked for a single day. It applies the 8:10 AM grace period, sets a 5:00 PM logout limit, and subtracts 1 hour for lunch.
Time and operators
